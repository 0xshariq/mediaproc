# Contributors

Thank you to everyone who has contributed to MediaProc! 🎉

## 🏆 Project Creator & Lead Maintainer

- **[@0xshariq](https://github.com/0xshariq)** - Original Author, Architecture & Lead Development

---

## 💻 Code Contributors

This project exists thanks to all the people who contribute.

<!--
Add contributors here as they join:

### Core Team
- [@username](https://github.com/username) - Description of contributions

### Contributors
- [@username](https://github.com/username) - Description of contributions
-->

_No additional contributors yet. Be the first!_

---

## 🌟 How to Become a Contributor

We welcome contributions from everyone! Here's how you can help:

1. **Code Contributions**

   - Fix bugs
   - Add new features
   - Improve documentation
   - Write tests
   - Create new plugins

2. **Non-Code Contributions**

   - Report bugs
   - Suggest features
   - Improve documentation
   - Help others in issues/discussions
   - Share MediaProc with others
   - Write tutorials or blog posts

3. **Getting Started**
   - Read our [Contributing Guide](CONTRIBUTING.md)
   - Check [open issues](https://github.com/0xshariq/mediaproc-cli/issues)
   - Join our [discussions](https://github.com/0xshariq/mediaproc-cli/discussions)

---

## 📜 Attribution

All contributors maintain their copyright on their contributions while agreeing to license them under the project's MIT License.

**Credit where credit is due:**

- Original concept and architecture: [@0xshariq](https://github.com/0xshariq)
- Each contributor retains copyright on their specific contributions
- All work is licensed under MIT License (see [LICENSE](LICENSE))

---

## 🙏 Special Thanks

- All our GitHub stargazers ⭐
- Everyone who reports bugs and suggests features
- The open-source community for inspiration and tools

---

<div align="center">

**Want to see your name here? [Start contributing today!](CONTRIBUTING.md)**

Built with ❤️ by the MediaProc community

</div>
