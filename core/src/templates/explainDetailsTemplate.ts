...
export function explainDetailsTemplate(context: ExplainContext): string {
    ...
    if (context.omittedFlags) {
        output += chalk.bold.bgGray.white('\n─────────────────────────── Omitted Flags ───────────────────────') + '\n';
        for (const [k, v] of Object.entries(context.omittedFlags)) {
            output += chalk.gray(`  - ${k}: ${v.defaultValue}`) + '\n';
        }
    }
    ...
    if (context.environment) {
        output += chalk.bold.bgGray.white('\n─────────────────────────── Environment ────────────────────────') + '\n';
        output += chalk.bold(`  cwd: ${context.environment.cwd}`) + '\n';
        output += chalk.bold(`  OS: ${context.environment.os}`) + '\n';
        output += chalk.bold(`  Node: ${context.environment.nodeVersion}`) + '\n';
        output += chalk.bold(`  Shell: ${context.environment.shell}`) + '\n';
    }
    output += chalk.gray('\nTip: Use --explain for a simple summary.');
    return output.trim();
}
